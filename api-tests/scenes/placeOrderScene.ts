import { OrdersApi, CreateOrderRequest, Order } from "../api/ordersApi";
...
export class PlaceOrderScene implements Scene {
  ...
  async run(): Promise<void> {
    ...
    const address = getStaticData<string>(
      this.context,
      "order.customer.address",
    );

    const city = getStaticData<string>(this.context, "order.customer.city");

    const postcode = getStaticData<string>(
      this.context,
