...
export class VerifyCreatedOrderScene implements Scene {
  static readonly key = "verifyCreatedOrder";
  ...
  async run(): Promise<void> {
    ...
    const expectedName = getStaticData<string>(
      this.context,
      "order.customer.name",
    );

    const expectedAddress = getStaticData<string>(
      this.context,
      "order.customer.address",
    );

    const expectedCity = getStaticData<string>(
      this.context,
      "order.customer.city",
    );

    const expectedPostcode = getStaticData<string>(
      this.context,
    ...
    expect(order.customer.name).toBe(expectedName);

    expect(order.customer.email).toBe(expectedEmail);

    expect(order.customer.address).toBe(expectedAddress);

    expect(order.customer.city).toBe(expectedCity);

    expect(order.customer.postcode).toBe(expectedPostcode);
    ...
    expect(orderItem.lineTotal).toBeCloseTo(product.price * quantity, 2);
